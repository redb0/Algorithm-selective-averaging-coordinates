"""
Тестовые функции храняться в виде словаря, 
в котором обязательно должны быть поля из списка required_fields.
Хранение информации в таком виде поддерживается для функций следующих типов:
1) функция Бочарова-Фельдбаума ('bocharov_feldbaum')
2) гиперболическая потенциальная функция ('hyperbolic_potential_abs')
3) гиперболическая потенциальная функция со степенями крутости = 2 ('hyperbolic_potential_sqr')
4) экспоненциальный потенциал ('exponential_potential')

Описание полей:
    'type'                    - тип функции, один из вышеописанных
    'index'                   - индекс 
    'number_extrema'          - количество экстремумов
    'coordinates'             - список координат экстремумов
    'func_values'             - список значений функций в точках экстремумах
    'degree_smoothness'       - список степеней гладкости функции в районе экстремума, 
                                чем больше значение тем положе будет район экстремума
    'coefficients_abruptness' - список коэффициентов крутости функции в районе экстремума
    'constraints_high'        - ограничения сверху для каждой координаты
    'constraints_down'        - ограничения снизу для каждой координаты
    'global_min'              - координаты глобального минимума
    'global_max'              - координаты глобального максимума
    'min_value'               - значение глобального минимума
    'max_value'               - значение глобального максимума
    'amp_noise'               - амплитуда сигнала (= max_value - min_value)
    'dimension'               - размерность функции/задачи оптимизации (количество координат)
