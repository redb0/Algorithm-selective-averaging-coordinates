...
class Options:
    ...
    Поля:
        _min_flag           : флаг минимизации (1 - минимизацияб 0 - максимизация)
        _max_iter           : максимальное число итераций, 
                              по достижению этого числа алгоритм принудительно будет остановлен
        _np                 : количество точек на каждой итерации
        _q                  : коэффициент q, рекомендуется = 1
        _g                  : коэффициент gamma, 
                              характеризует скорость сжатия области поиска, рекомендуется 0.8 <= g <= 1.2
        _s_factor           : коэффициент селективности
        _idx_nf             : индекс яделной функции (подробнее см. ./nuclear_function.py)
        _k_noise            : коэффициент "шум/сигнал",
                              если == 0, то помехи отсутствуют, иначе амплитуда, 
                              накладываемой помехи, будет равна amp * k_noise (amp берется из описания тестовой функции)
        _initialization_mode: режим инициализации начального положения рабочей точки,
                              Поддерживаются значения: 
                                  'center' - рабочая точка будет располагаться в центре области поиска
                                  'random' - случайное расположение внутри области поиска
                                  'manual' - вручную установить координаты.
        _x                  : начальные координаты рабочей точки, необходимо установить если выбран режим 'manual'
